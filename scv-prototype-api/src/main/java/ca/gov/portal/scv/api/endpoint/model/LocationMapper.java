...
public class LocationMapper {
	...
	public Location map(ca.gov.portal.scv.api.service.dto.Location location) {
		return Location.builder()
			.city(location.getLocationAddress().getCityName())
			...
			.line1(mapLine1(location))
			.postalCode(location.getLocationAddress().getPostalCode())
			.provinceCode(location.getLocationAddress().getState().getStateCode())
			.provinceName(mapProvinceName(location))
			.build();
	}
	...
	protected Map<String, String> mapProvinceName(ca.gov.portal.scv.api.service.dto.Location location) {
		...
		final String englishStateName = stateNames.stream()
			.filter(stateName -> stateName.containsKey("eng"))
			.map(stateName -> stateName.get("eng")).findFirst()
			.orElse("");

		final String frenchStateName = stateNames.stream()
			.filter(stateName -> stateName.containsKey("fra"))
			.map(stateName -> stateName.get("fra")).findFirst()
			.orElse("");
