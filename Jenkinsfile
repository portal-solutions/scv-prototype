				...
				stage('Build API') {
					...
					steps {
						dir(path: 'scv-prototype-api') {
							...
							sh 'docker login -u $AZURE_CR_CREDS_USR -p $AZURE_CR_CREDS_PSW portalsolutions.azurecr.io && docker push portalsolutions.azurecr.io/portal-solutions/scv-prototype-api'
